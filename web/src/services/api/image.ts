...
export function imageResponseOptions(model: string) {
    const normalized = model.trim().toLowerCase();
    return normalized.startsWith("gpt-image-") || normalized === "chatgpt-image-latest" ? { output_format: IMAGE_OUTPUT_FORMAT } : { response_format: "b64_json" };
}

export function appendImageFiles(formData: FormData, files: File[]) {
...
function parseImageRatio(value: string) {
    ...
    if (!Number.isFinite(w) || !Number.isFinite(h) || w <= 0 || h <= 0) throw new Error("Image ratio must be positive numbers, e.g. 9:16");
    if (Math.max(w, h) / Math.min(w, h) > IMAGE_MAX_RATIO) throw new Error("Image aspect ratio cannot exceed 3:1; adjust the size");
...
function validateImageSize(width: number, height: number) {
    ...
    if (width % IMAGE_SIZE_STEP !== 0 || height % IMAGE_SIZE_STEP !== 0) throw new Error("Image width and height must be multiples of 16; adjust the size");
    if (Math.max(width, height) > IMAGE_MAX_EDGE) throw new Error("Image longest edge cannot exceed 3840px; adjust the size");
    if (Math.max(width, height) / Math.min(width, height) > IMAGE_MAX_RATIO) throw new Error("Image aspect ratio cannot exceed 3:1; adjust the size");
    const pixels = width * height;
    if (pixels < IMAGE_MIN_PIXELS || pixels > IMAGE_MAX_PIXELS) throw new Error("Total image pixels must be between 655360 and 8294400; adjust the size");
}
...
function parseGeminiImagePayload(payload: GeminiPayload) {
    ...
    const images =
        payload.candidates
            ...
            .filter((value): value is string => Boolean(value))
            .map((dataUrl) => ({ id: nanoid(), dataUrl })) || [];
    if (!images.length) throw new Error("The Gemini API did not return an image");
    return images;
}
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    ...
    if (requestConfig.apiFormat === "gemini") {
        if (mask) throw new Error("Gemini API format does not support mask editing yet");
        try {
    ...
    Object.entries(imageResponseOptions(requestConfig.model)).forEach(([key, value]) => formData.set(key, value));
    if (quality) {
...
export async function fetchImageModels(config: Pick<AiConfig, "baseUrl" | "apiKey" | "apiFormat">) {
    try {
        ...
        throw new Error(readAxiosError(error, "Failed to read models"));
    }
}
