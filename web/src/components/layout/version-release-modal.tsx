...
function getTagColor(type: string) {
    ...
    if (type === "Fixes") return "red";
    if (type === "Changed") return "blue";
    if (type === "Docs") return "purple";
    return "default";
}

function getReleaseTitle(version: string) {
    return version === "Unreleased" ? "Unreleased" : version;
}
...
export function VersionReleaseModal({ className, style }: VersionReleaseModalProps) {
    ...
    return (
        <>
            ...
            <Modal title="Version updates" open={open} width={680} centered footer={null} onCancel={() => setOpen(false)}>
                <div className="mb-5 grid grid-cols-2 gap-3">
                    <div className="rounded-lg border border-stone-200 p-3 dark:border-stone-800">
                        <div className="text-xs text-stone-500 dark:text-stone-400">Current version</div>
                        <div className="mt-1 text-base font-semibold text-stone-950 dark:text-stone-100">{APP_VERSION}</div>
                    </div>
                    <div className="rounded-lg border border-stone-200 p-3 dark:border-stone-800">
                        <div className="flex items-center justify-between gap-3">
                            <div className="text-xs text-stone-500 dark:text-stone-400">Latest version</div>
                            <button
                                type="button"
                                ...
                                <div>
                                    <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        <div className="flex min-w-0 items-center gap-1.5">
                                            {release.version === latestVersion ? <Tag color="green">Latest</Tag> : null}
                                            {release.version === APP_VERSION ? <Tag>Current</Tag> : null}
                                        </div>
                                    </div>
